\documentclass[10pt, xcolor={dvipsnames, table}]{beamer}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{booktabs}
\usepackage[style=authortitle,
            autocite=footnote,
            backend=biber,
           ]{biblatex}
\usepackage{multirow}
\usepackage{adjustbox}
\usepackage[font=scriptsize]{caption}
\usepackage{subcaption}
\usepackage{hyperref}
\usepackage[ruled]{algorithm2e}
\usepackage{algorithmic,float}
\usepackage{dirtytalk}
\usepackage{marginnote}
\usepackage{tikz}
\usepackage[T1]{fontenc}
\usepackage{transparent}
\usepackage{eso-pic}
\usepackage{lipsum}
\usepackage{wrapfig}
\usepackage{tabularx}
\usepackage{soul}
\usepackage{array}
\usepackage[absolute,overlay]{textpos}
  \setlength{\TPHorizModule}{1mm}
  \setlength{\TPVertModule}{1mm}
%\usepackage{MnSymbol}
\usepackage{bm}


\newcolumntype{M}[1]{>{\centering\arraybackslash}m{#1}}

\SetKwInOut{Parameter}{parameter}

\captionsetup[figure]{labelsep=period}
\captionsetup[subfigure]{labelformat=simple}
\renewcommand\thesubfigure{\thefigure.\alph{subfigure}.}


\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{caption}[number]
\bibliography{Bibliography}

\graphicspath{{/Users/flavioforenza/Desktop/latex/images/}}

% \defbeamertemplate{footline}{left page number}
% {%
%   \hspace*{0.2cm}%
%   \usebeamercolor[fg]{page number in head/foot}%
%   \usebeamerfont{page number in head/foot}%
%   \insertpagenumber\,/\,\insertpresentationendpage%
%   \hspace*{0.2cm}\vskip2pt%
% }
% \setbeamertemplate{footline}[left page number]
\setbeamertemplate{footline}[frame number]
\definecolor{darkblue}{RGB}{ 0, 0, 139}

\usetheme{Darmstadt}


%\title[short]{UNIVERSITÀ DEGLI STUDI DI MILANO}
\title[]{Distilled-Single-Shot-Detector (DSSD): un nuovo modello di guida autonoma ad alta inferenza}

\author[Flavio Forenza]{
                        \includegraphics[scale = 0.05]{
                                        unimilogo.png}\\ 
                                        \vspace{0.1cm}
                                        \fontfamily{cmr}\selectfont {\bfseries{UNIVERSITÀ DEGLI STUDI DI MILANO}}\\
                                        \fontfamily{cmr}\selectfont \emph{Laurea magistrale in Informatica}\\
                                        \vspace{1cm}
                                        }
\date{\scriptsize 21 Aprile 2022}

\begin{document}

\begin{frame}
    \maketitle
    \vspace{-2.5cm}
    \begin{minipage}{\linewidth}
        \centering
        \begin{minipage}{0.45\linewidth}
            \begin{flushleft}
                \emph{Relatore:}\\
                Prof. Vincenzo Piuri\\
                \emph{Correlatore:}\\
                Dott. Angelo Genovese
            \end{flushleft}
        \end{minipage}
        \begin{minipage}{0.45\linewidth}
            \begin{flushright}
                \emph{Laureando:}\\
                Flavio Forenza
            \end{flushright}
        \end{minipage}
    \end{minipage}
\end{frame}

\logo{{\vspace{-1.5cm}\reflectbox{\hspace{-1cm}\rotatebox[origin=c]{-40}{\fontsize{20mm}{15mm}\selectfont{\transparent{0.3}\includegraphics[width=0.2\linewidth]{unimilogo_reflect.png}}}}}}
\newcommand{\nologo}{\setbeamertemplate{logo}{}}

\section{"Distilled-Single-Shot-Detector (DSSD): un nuovo modello di guida autonoma ad alta inferenza"}
%\input{frames/objectives.tex}
\input{frames/crisis_semi.tex}
\input{frames/thesis_job.tex}
%\input{frames/perfomance.tex}

\input{frames/compression_tech.tex}
\input{frames/pruning.tex}
\input{frames/KD.tex}

\input{frames/KD_steps.tex}

\input{frames/stud_teach.tex}
\input{frames/proposed_model.tex}

\input{frames/architectures.tex}
\input{frames/dataset.tex}

\input{frames/results_pruning.tex}
\input{frames/KD_dimension.tex}
\input{frames/speed_DSSD.tex}
\input{frames/example.tex}
\input{frames/conclusion.tex}
\input{frames/end.tex}
%\input{frames/table.tex}

\end{document}