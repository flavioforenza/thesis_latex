...
minimo e media. Così come la convoluzione\index{convoluzione}, anche qui gli iperparametri\index{iperparametri} Stride\index{stride} 
e Kernel\index{kernel} size, entrambi solitamente uguale a 2, sono fondamentali al fine di gestire 
...
\begin{figure}
    ...
    \label{pooling}
\end{figure}
In (Fig. \ref{pooling}), applicando al funzione di \emph{Max-Pooling}\index{pooling} al campo ricettivo\index{campo ricettivo} in 
basso a sinistra, verrà selezionato il valore 5 che sarà propagato al prossimo 
layer\index{layer}. Avendo uno stride\index{stride} pari a 2, la matrice prodotta avrà una dimensione 
...
procedura di \emph{Upsampling}, viene effettuata un'operazione di \emph{deconvoluzione}\index{deconvoluzione}, spesso 
...
\begin{itemize}
    ...
    La sua struttura è composta da livelli di convoluzione\index{convoluzione} e di max 
    ...
    di deconvoluzione\index{deconvoluzione} utili a ricostruire l'immagine di ingresso includendone i 
    suoi artefatti.
\end{itemize}
...
(upsampling), o di convoluzione\index{convoluzione} e deconvoluzione\index{deconvoluzione}, sono mostrate nella 
...
perdita di dati rilevanti. L'idea della Convoluzione\index{convoluzione} dilatata, chiamata anche 
...
tradizionale operatore di convoluzione\index{convoluzione}, la convoluzione dilatata introduce un tasso 
di dilatazione \emph{(dilated rate)} $l$, utile a saltare alcuni punti, presenti nel 
campo ricettivo\index{campo ricettivo}, durante la convoluzione\index{convoluzione}. La formula matematica per il calcolo della 
...
\begin{equation}\label{dilated rate}
    Conv(l) = \left\{
        ...
        \right.
\end{equation}
Con un dilated rate pari a 2, otterremmo la seguente convoluzione\index{convoluzione} dilatata (Fig. (\ref{dilated})):
\begin{figure}
    \centering
    ...
    \label{dilated}
\end{figure}
Maggiore sarà il valore del tasso di dilatazione, maggiore sarà il campo ricettivo\index{campo ricettivo} 
...
Per poter ricostruire le informazioni perse, si può utilizzare la deconvoluzione\index{deconvoluzione}. 
Quest'ultima richiede però un quantitativo importante di memoria e di tempo. 
L'algoritmo offre un metodo alternativo alla deconvoluzione\index{deconvoluzione}. Il suo scopo 
è quello di applicare una convoluzione\index{convoluzione} che permetta di allargare efficacemente il 
campo visivo dei filtri senza aumentare il numero di parametri\index{parametro} preservando la 
quantità di computazione richiesta. La tecnica della convoluzione\index{convoluzione} atrosa può essere 
...
mentre la parte destinata al decoder contiene anch'essa 13 layer per la deconvoluzione\index{deconvoluzione}. 
...
inferiore, viene eseguita una normalizzazione L2, a ciasun livello di convoluzione\index{convoluzione}, 
...
di upsampling con un layer\index{layer} di deconvoluzione\index{deconvoluzione}. Le mappe ottenute dalla 
...
un encoder e un decoder con una convoluzione\index{convoluzione} dilatata separata. L'encoder 
