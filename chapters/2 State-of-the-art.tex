...
locale del layer\index{layer} precedente e da qui ne estrae le feature\index{feature} locali. Un filtro, che in 
gergo prende il nome di {\bfseries{\emph{kernel}}}, trasla sull'immagine originale con l'intenzione di 
creare una feature map\index{feature map} mediante moltiplicazione tra i valori presenti nel filtro, 
corrispondenti ai pesi\index{peso}, e la regione dell'immagine in cui si trova. Di seguito, i 
valori di ogni feature map\index{feature map} sono calcolati tramite la seguente formula:
...
feature map\index{feature map}. Il numero di feature map\index{feature map} sarà uguale al numero di filtri applicato 
sull'immagine. La generazione di ogni feature map\index{feature map} dipende da quattro tipi di 
...
\begin{enumerate}
    ...
    \begin{figure}
        ...
        \label{stride}
    \end{figure}
    \item \emph{Riempimento (Padding)}: affinché venga generata una corretta feature map\index{feature map} 
...
Ogni feature map\index{feature map}, dopo la convoluzione, è composta da dei valori positivi e negativi. 
...
La tecnica di Pooling\index{pooling}, chiamata anche di subsampling o downsamping, ha il 
compito di ridurre la dimensionali di ogni feature map\index{feature map} mantenendo però le 
...
alla grandezza della feature map\index{feature map}, con lo scopo di aggregare i valori sottostanti. 
...
la finestra sovrastante. Ogni neurone\index{neurone}, nello strato di pooling\index{pooling}, è connesso a un 
...
\begin{figure}
    ...
    \label{pooling}
\end{figure}
In (Fig. \ref{pooling}), applicando al funzione di \emph{Max-Pooling}\index{pooling} al campo ricettivo in 
...
minore di paramenti da apprendere. La funzione di Max-Pooling\index{pooling} introduce quella che viene 
...
Il layer\index{layer} \emph{Fully Connected (FC)}\index{fully connected} è una tradizionale \emph{Multilayer Perceptron (MLP)} 
che include una funzione di attivazione\index{funzione di attivazione} \emph{Softmax} nel layer di output. Lo scopo di 
tale layer\index{layer} è quello di utilizzare le feature\index{feature} estratte, dagli strati precedenti, in modo 
...
connessione simile alle CNN\index{Convolutional Neural Network - CNN} che li permette di retro propagare i dati in direzione 
...
sbagli la sua predizione. Tramite un continuo processo di back and forward\index{forward}, 
...
L'introduzione delle reti neurali, sia nel campo della computer vision\index{computer vision} che del deep 
...
classificazione\index{classificazione} in grado di definire se un'immagine contiene un oggetto e, in caso 
...
\begin{figure}
    ...
    \label{segmentation}
\end{figure}
Uno dei principali obiettivi in questo campo è scegliere le feature\index{feature} che sono 
...
il loro funzionamento su delle feature\index{feature} estratte a mano \cite{viola2001rapid} \cite{dalal2005histograms}. Purtroppo, questi 
modelli erano molto lenti, imprecisi e non erano in grado di riconoscere i dati mai 
visti in precedenza. Le reti neurali convoluzionali (CNN)\index{Convolutional Neural Network - CNN} e del deep learning\index{deep learning} hanno 
...
computer vision\index{computer vision} sono stati raggiunti traguardi importanti negli ultimi dieci anni, 
...
Networks (DCNN)}, in particolare dalle Convolutional Neural Networks (CNN)\index{Convolutional Neural Network - CNN}. 
...
la strada verso il continuo successo delle CNN\index{Convolutional Neural Network - CNN} nello svolgere vari compiti di visione 
artificiale di alto livello, oltre che a motivare i ricercatori ad esplorare le capacità 
di tali reti per problemi di classificazione\index{classificazione} pixel-wise, come la segmentazione delle 
...
della classificazione\index{classificazione} e dell'object detection\index{object detection}, è necessario reperire delle informazioni 
...
convoluzionali, fino a giungere ai layer\index{layer} fully connected (FC)\index{fully connected}. L'output prodotto 
sarà sotto forma di label predetta (Fig. \ref{CNN complete}). Per convertire questi modelli\index{modello} da 
classificatori a modelli FCN densi, bisogna sostituire i layer fully connected\index{fully connected} con 
...
\end{enumerate}
Quando si sarà giunti a un layer\index{layer} di pooling\index{pooling} pari alla dimensione di $1 \times 1$, allora 
...
\begin{itemize}
    ...
    pooling\index{pooling}, come una comune CNN\index{Convolutional Neural Network - CNN}, che riducono (down-sample) l'immagine 
    in ingresso; 
    ...
    di un decoder è quello di amplificare le feature map\index{feature map} tramite operazioni 
...
svolte, un confronto tra l'operazione di Pooling\index{pooling} (downsampling) e di Unpooling\index{unpooling} 
...
map, utilizzano i livelli di pooling\index{pooling} precedenti nella quale ci sono maggiori informazioni. 
...
tra l'upsampling del pooling\index{pooling} layer\index{layer} $n$ con il pooling layer $n-1$ (Fig. (\ref{FCN-models})). La ricostruzione 
...
Per rimediare alla bassa qualità prodotta in output da una comune rete CNN\index{Convolutional Neural Network - CNN}, Yu 
...
alla riduzione della risoluzione delle feature\index{feature}, è stato risolto grazie all'applicazione 
dell'algoritmo soprannominato “atrous”. Ogni encoder è composto dalla combinazione 
di layer\index{layer} di pooling\index{pooling} che riducono la risoluzione spaziale delle feature map\index{feature map}. 
...
feature map\index{feature map} seguita da una deconvoluzione $2 \times 2$. La mappa delle caratteristiche 
...
degli indici di pooling\index{pooling} per ciascuna feature map\index{feature map} prodotta dall'encoder. 
...
La feature map\index{feature map} prodotta da un decoder verrà inserità in un classificatore softmax 
...
normalizzazioni L2 (\ref{L2}), una dopo il pooling\index{pooling} globale e l'altra dopo la feature 
...
medio delle feature map\index{feature map} in ogni specifico livello di convoluzione. Dopo aver 
ottenuto le feature globali, viene effettuata una normalizzazione L2. L'unpooling\index{unpooling} 
...
esempio una traslazioni o rotazioni, rende la classificazione\index{classificazione} invariante mentre, allo 
stesso tempo, la localizzazione\index{localizzazione}  potrebbe risentirne negativamente, pertanto risulta 
essere più sensitiva rispetto alla classificazione\index{classificazione}. Per sopperire alle problematiche 
...
\begin{enumerate}
    ...
    \item Dal punto di vista della \emph{classificazione}\index{classificazione}, devono essere utilizzati dei kernel 
...
effettua una convoluzione dilatata ad ogni feature map\index{feature map} di input che unirà con le 
...
\begin{enumerate}
    ...
    \item \emph{Rilevamento\index{rilevamento} delle feature\index{feature} (Feature Detection)}: l'obiettivo principale 
...
of Oriented Gradient (HOG)}) e \emph{Haar-like}, quest'ultime sono feature\index{feature} che 
...
\begin{enumerate}
    ...
    interesse (ROI) che verrano inviate al modulo di classificazione\index{classificazione}. Lo 
...
classificazione\index{classificazione} riguarda al determinazione del tipo di segnale che il sistema 
...
feature\index{feature} è la trasformata di Hough o le già citate wavelet di Haar. Un 
...
l'eliminazione del kernel, corrispondente a tale feature maps\index{feature map}, presente nel 
