...
La tecnica di Pooling\index{pooling}, chiamata anche di subsampling o downsampling, ha 
...
L'image segmentation\index{image segmentation} si suddivide in due categorie:
\begin{enumerate}
...
non si limita solamente a classificare gli oggetti presenti (image classification\index{image classification}) e a 
rilevarne la loro posizione (object detection)\index{object detection} mediante delle bounding box\index{bounding box}, ma 
...
\begin{figure}
    ...
    \label{FCN}
\end{figure}
Tale architettura\index{architettura} è composta da un \emph{Encoder}\index{encoder} e da un \emph{Decoder}\index{decoder}.
\begin{itemize}
    \item \emph{Encoder}\index{encoder}: sezione dell'architettura\index{architettura} adibita all'estrazione del contesto dell'immagine. 
    ...
    in ingresso; 
    \item \emph{Decoder}\index{decoder}: sezione dell'architettura\index{architettura} adibita alla localizzazione\index{localizzazione}  degli elementi 
    presenti nell'immagine attraverso l'utilizzo di convoluzioni trasposte. Il compito 
    di un decoder\index{decoder} è quello di amplificare le feature map\index{feature map} tramite operazioni 
...
un encoder\index{encoder} che ha il compito di ridurre l'immagine iniziale di 32 volte e da un 
decoder\index{decoder} che svolgerà il processo inverso di ricostruzione. Avendo pochi dati a 
...
dall'encoder\index{encoder}. Per avere una rappresentazione più fedele dell'immagine in output, 
...
risultante sarà quella più fedele al ground truth\index{ground truth}. Questa architettura\index{architettura} 
...
dell'algoritmo soprannominato “atrous”. Ogni encoder\index{encoder} è composto dalla combinazione 
...
Tali indici saranno utili al decoder\index{decoder} per effettuare l'operazione di upsampling 
