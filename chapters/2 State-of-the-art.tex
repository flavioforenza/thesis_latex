...
map\index{feature map} dipende da quattro tipi di parametri\index{parametro}, detti \emph{iperparametri}\index{iperparametro}:
\begin{enumerate}
    ...
    \end{figure}
\end{enumerate}
La scelta degli iperparametri\index{iperparametro} non è standard ma dipende maggiormente 
...
qui gli iperparametri\index{iperparametro} Stride\index{stride} e Kernel\index{kernel} size, entrambi solitamente uguale 
...
\begin{itemize}
    ...
    su dispostivi sprovvisti di elevate risorse\index{risorsa} di calcolo. La ricerca è rivolta 
...
termocamere e sistemi per la stima della profondità (es: LIDAR)\index{ligth detector and range (LIDAR)}. Diversi 
...
rapporto di chiusura degli occhi (\emph{eye closure ratio})\index{eye closure ratio}, il battito delle palpebre 
...
sono stati proposti diversi approcci \cite{salama2019pruning} utili a mitigare la sparsità\index{sparsità} di tali 
...
\begin{enumerate}
    ...
    rimuovere interi pesi\index{peso} introducendo quasi sempre un'elevata sparsità\index{sparsità} 
    ...
    della sparsità\index{sparsità}. A questo proposito, \cite{li2017pruning} propose una "\emph{channel-wise 
    pruning}" in accordo con la norma \emph{L1}\index{norma L1} del filtro corrispondente. La 
    ...
    unstructured pruning consente di potare i layers meno utili (entropia\index{entropia} 
    inferiore) rispetto a quelli più utili (entropia\index{entropia} più elevata).
\end{enumerate}
...
queste tecniche utilizzano le norme L1\index{norma L1} (Geometria dei Taxi/Distanza di 
Manhattan) e L2\index{norma L2} (Distanza Euclidea) come regolarizzatori. Nello specifico, 
...
"\emph{The lottery ticket hypothesis}"\index{lottery ticket hypothesis} \cite{frankle2019lottery} ha intensificato l'interesse per le tecniche 
...
Sempre secondo \cite{frankle2019lottery}, l'introduzione di una crescente percentuale di sparsità\index{sparsità} 
...
dispositivi con risorse\index{risorsa} limitate come la videosorveglianza e la guida autonoma\index{autonoma}. 
...
per migliorare l'accuratezza\index{accuratezza} di una piccola rete, chiamata "\emph{Studente}", trasferendo 
...
argomento con il concetto di "Transfer Learning"\index{transfer learning}. L'obiettivo del Transfer 
...
cosiddetti "\emph{soft targets}"\index{soft targets}. Utilizzando i logits\index{logits} del modello\index{modello} insegnante, che 
...
suddetti logits\index{logits} come conoscenza distillata, con lo scopo di ridurre al minimo 
l'entropia\index{entropia} incrociata tra i logits\index{logits} dell'insegnante e i propri logits prodotti. 
Prima di introdurre il concetto principale, bisogna ricordare come una 
funzione Softmax (o funzione di attivazione)\index{funzione di attivazione} trasformi i numeri (logits\index{logits}) in 
...
\begin{itemize}
    ...
    \item $z_J$ logit\index{logits} corrispondente alla classe $j$-esima;
...
inputs. Le probabilità risultati vengono chiamate "\emph{hard-targets}\index{hard targets}" (o hard 
...
dove:
\begin{itemize}
    \item $T>1$: indica un parametro\index{parametro} chiamato \emph{Temperatura}\index{temperatura} \cite{marino2021compact}.
\end{itemize}
Le probabilità risultanti da questa softmax modificata, prenderanno il nome 
di "\emph{soft-targets}"\index{soft targets} (o soft-labels). Arrivati a questo punto, c'è da porsi la 
seguente domanda: Qual'è il significato delle soft-targets?\index{soft targets} Per comprendere 
...
di distinzione di tali numeri vuol dire avere una buona "\emph{generalizzazione}"\index{generalizzare}. 
\begin{displayquote}
...
generalizzazione\index{generalizzare}. La temperatura\index{temperatura} è utile a controllare l'importanza di ogni 
...
è diffuso sotto il nome di "\emph{Dark Knowledge}"\index{Knowledge Distillation} (Conoscenza Oscura)\index{dark knowledge}. Un 
...
sia con le soft-targets\index{soft targets} che con le hard-targets\index{hard targets}, gli autori hanno utilizzato la 
somma ponderata di due perdite. La prima perdita ($L_{soft}$) è quella ricavata 
dal calcolo dell'entropia incrociata (cross-entropy loss)\index{entropia} tra le previsioni del 
modello\index{modello} studente e le label corrette fornite dal ground-truth\index{ground truth}. La seconda 
funzione di perdita ($L_{hard}$) invece si basa sul calcolo di un'entropia\index{entropia} incrociata 
tra le soft-targets\index{soft targets} prodotte da entrambi i modelli, studente e insegnante, 
alla stessa quantità di temperatura\index{temperatura}. Matematicamente parlando, denotiamo 
corrispettivamente con $v_j$ e $z_j$ i logits\index{logits} di insegnante e studente. Dopo esserci 
calcolati le probabilità soft-targets\index{soft targets}  $q_j$ e $p_j$ derivanti dalla softmax dello 
studente e dell'insegnante, ad una stessa temperatura\index{temperatura} T, possiamo definire 
la seconda funzione di perdita, ovvero la seconda entropia\index{entropia} incrociata, con la 
seguente formula:
\begin{equation}\label{cross_entropy_loss}
    C(z,v) = -\sum_jp_j \log{q_j}
\end{equation}
Il gradiente\index{gradiente} derivante dal calcolo della derivata $dC/dz_J$ sull'entropia\index{entropia} incrociata 
...
\end{equation}
La Formula (\ref{cross_entropy_soft}) permette di minimizzare l'entropia\index{entropia} incrociata (\ref{cross_entropy_loss}) 
e di migliorare le performance\index{performance} dello studente sfruttando la capacità di 
...
\end{equation}
Poichè i magnitudi dei gradienti prodotti dai soft-targets\index{soft targets} scalano a $1/T^2$, 
...
(\ref{l_hard_soft}). Da come possiamo vedere, la funzione di perdita $L_{hard}$ è generata 
nel blocco "Distillation", mentre la funzione di perdita $L_{soft}$ dello studente 
