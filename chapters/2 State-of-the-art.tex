...
di questi insiemi abbiamo la presenza di esempi formati da coppie $(x^q, y^q)$, con 
$q=1,…,n$, dove $y^q$ sta a rappresentare il valore di output desiderato in funzione 
del dato di input $x^q$. Per ricavare il valore target è necessario ricercare gli opportuni 
valori dei pesi, indispensabili a minimizzare l’errore commesso. Quest’ultimo è 
associato ad una la \emph{Funzione di Costo} (o di perdita) (\ref{cost function}):
\begin{equation}\label{cost function}
    \emph{C}=(a_k-y)^2
\end{equation}
Si può osservare che il calcolo di (\ref{cost function}) è riconducibile alla somma dei quadrati 
residua tra il valore stimato e quello desiderato. La funzione di costo $C$ risulta 
essere molto importate nella fase di apprendimento in quanto misura la distanza 
dalla soluzione ottimale. In generale, la funzione di costo ha diversi minimi (Fig. \ref{minimum and maximum})
\begin{figure}[H]
    \centering
    \includegraphics[width = 0.7\linewidth]{cost-function.png}
    \centering
    \caption{Esempio di minimo globale e locale della funzione di costo.}
...
ottimali avviene mediante calcolo delle derivate parziali sulla funzione di costo, 
ovvero del suo vettore gradiente $\nabla{C}$. Questo vettore è utile a definire la direzione 
ed il verso della funzione di costo, in base al set di pesi considerato. Il calcolo delle 
