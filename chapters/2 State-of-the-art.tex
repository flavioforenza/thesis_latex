...
Lo scambio di dati tra i vari neuroni\index{neurone} consente alla rete neurale di poter generalizzare 
anche con dati mai visti nel training set. Questo processo prende il nome di 
“\emph{Apprendimento}”. L'apprendimento\index{apprendimento} di un modell\index{modello} di rete è tipicamente effettuato 
...
opportuni valori dei pesi\index{peso}, indispensabili a minimizzare l'errore commesso. La 
quantità dell'errore commesso, dal singolo neurone\index{neurone} $j$, è esprimibile mediante (\ref{error function}):
...
La funzione di errore $E$ risulta essere molto importate nella fase di apprendimento\index{apprendimento} in quanto misura la distanza 
...
da dei pesi\index{peso} a valori casuali, fino a definirne dei valori fissi. La ricerca dei pesi 
...
in base al set di pesi\index{peso} considerato. Il calcolo delle derivate viene svolto da una ben 
...
In questa tipologia di apprendimento\index{apprendimento}, l'input fornito alla rete contiene una serie di 
dati etichettati. L'apprendimento supervisionato è solitamente utilizzato sia nel 
conteso della classificazione, dove  si vuole mappare le etichette\index{etichetta} di input a quelle 
...
da parte del modello\index{modello}. L'apprendimento\index{apprendimento} migliora grazie ad una continua variazione 
dei pesi\index{peso}. Questo tipo di apprendimento è svolto utilizzando la tecnica della 
...
di overfitting della rete. In questa categoria di apprendimento\index{apprendimento} fanno parte diversi metodi deep 
learning\index{deep learning}, incluse le \emph{Deep Neural Networks (DNN)}, le \emph{Convolutional Neural 
Networks (CNN)}\index{Convolutional Neural Netwrok - CNN}, le \emph{Recurrent Neural Networks (RNN)} come le \emph{Long 
...
Un set di input composto da dati etichettati\index{etichetta} e non, costituisce un sistema di apprendimento 
semi-supervisionato. Questa tipologia di apprendimento\index{apprendimento} rappresenta 
...
e una grande quantità di dati non etichettati. Tale apprendimento\index{apprendimento} porta alla 
creazione di un modello\index{modello} più flessibile rispetto a quello ottenuto dall'apprendimento\index{apprendimento} 
...
\emph{agente}, che abbia l'obiettivo di migliorare le sue performance\index{performance} interagendo con 
...
con etichette\index{etichetta} o valori di verità, ma rappresentano un livello di qualità delle azioni 
...
una variazione della funzione di errore E. Tralasciando un'attimo il bias\index{bias}, il discorso 
...
dei pesi, per un singolo peso\index{peso} $w_{ji}$. Questa operazione, in letteratura, è nominata 
...
\begin{eqnarray}\label{local gradient of output layer}
    \delta_j & = & \frac{\partial E}{\partial a_{j}} \nonumber \\
             ...
             & = & e_j\varphi_j^{'}(a_j)
\end{eqnarray}
Il gradiente locale sta ad indicare i cambiamenti richiesti nei pesi\index{peso} e questo dipende 
...
maggiormente sui pesi\index{peso}. Essendo la funzione di errore sensibile anche al cambiamento 
del bias\index{bias}, l'algoritmo di back-propagation ha lo scopo di aggiornare anche i 
vari bias\index{bias} esistenti integrando, nel vettore gradiente $\nabla{E}$, le derivate parziali della 
funzione di Errore rispetto al bias\index{bias} $b$:
...
\begin{enumerate}
    ...
    \item \emph{Convolutional Neural Netoworks (CNN)}\index{Convolutional Neural Netwrok - CNN}
...
l'ultimo layer\index{layer}. Rispetto a una rete semplice, una rete feedforward può essere 
composta da più layer nascosti in cui vi sono più neuroni\index{neurone} nascosti. L'aggiunta 
di un numero sostanziale di layer\index{layer} nascosti può aiutare ad estrarre statistiche di 
ordine superiore dai dati di input. Secondo \cite{04}, la presenza di più connessioni 
sinaptiche tra i neuroni\index{neurone}, provoca una maggiore iterazione tra questi che a sua 
volta aiuta alla rete a raggiungere una prospettiva globale. Una rete che ha più di 
un layer\index{layer} nascosto, e quindi probabilmente con un alto numero di connessioni, in 
...
Una rete neurale convoluzionale (CNN o ConvNet)\index{Convolutional Neural Netwrok - CNN} è una tipologia di rete feed-
...
negli anni '90 grazie a uno studio di LeCun e Boser \cite{NIPS1989_53c3bce6}. L'architettura\index{architettura} delle CNN\index{Convolutional Neural Netwrok - CNN} 
si distinguono grazie al numero di layer\index{layer} presenti.  La loro particolarità sta nel 
...
neuroni:
\begin{itemize}
    \item \emph{Neuroni\index{neurone} di elaborazione}: hanno lo scopo di elaborare una sezione specifica 
    dell'immagine, comunemente chiamata campo ricettivo, utilizzando 
    un'operazione di \emph{convoluzione};
    \item \emph{Neuroni\index{neurone} di aggregazione}: hanno lo scopo di eseguire un'operazione di \emph{pooling}.
\end{itemize}
I neuroni contenuti in uno strato adibito alla convoluzione, e quelli completamente 
connessi, dispongono di funzioni di attivazione\index{funzione di attivazione} e di pesi, quest'ultimi sottoposti a 
processo di aggiornamento durante la fase di apprendimento\index{apprendimento}. Per quanto riguarda 
gli strati di pooling, questi non dispongo di alcun tipi di peso\index{peso}. Riassumendo, 
le operazioni svolte dalle CNN\index{Convolutional Neural Netwrok - CNN} sono essenzialmente quattro: {\bfseries{Convoluzione}}, {\bfseries{Non 
Linearità (ReLU)}}, {\bfseries{Pooling (o Sub Sampling)}} e {\bfseries{Classificazione}} (nei layer\index{layer} 
fully connected). 
\begin{figure}
    \centering
...
Lo scopo del layer\index{layer} di convoluzione è quello di attuare un'operazione di {\bfseries{feature 
extraction}}. In questa fase ogni neurone\index{neurone} prende in input un campo ricettivo 
locale del layer\index{layer} precedente e da qui ne estrae le feature locali. Un filtro, che in 
...
corrispondenti ai pesi\index{peso}, e la regione dell'immagine in cui si trova. Di seguito, i 
...
parametri\index{parametro}, detti 
\emph{iperparametri}:
\begin{enumerate}
...
Come brevemente accennato in (\ref{Reti neurali}), esistono varie funzioni di attivazione\index{funzione di attivazione} che, in 
base all'input ricevuto, decidono se attivare o disattivare un neurone\index{neurone}. La 
funzione di attivazione\index{funzione di attivazione} maggiormente usata è la ReLU (Rectified Linear Unit). 
...
il processo di apprendimento\index{apprendimento} (backpropagation) del modello\index{modello} semplificando 
le operazioni nei vari layer\index{layer}.
\begin{figure}
    \centering
...
senza alcun tipo di valore o peso\index{peso} al suo interno, di dimensione inferiore rispetto 
...
la finestra sovrastante. Ogni neurone\index{neurone}, nello strato di pooling, è connesso a un 
...
layer\index{layer}. Avendo uno stride pari a 2, la matrice prodotta avrà una dimensione 
...
parametri\index{parametro}, limitando anche il rischio di overfitting in quanto si ha un numero 
...
Il layer\index{layer} \emph{Fully Connected (FC)} è una tradizionale \emph{Multilayer Perceptron (MLP)} 
che include una funzione di attivazione\index{funzione di attivazione} \emph{Softmax} nel layer di output. Lo scopo di 
tale layer\index{layer} è quello di utilizzare le feature estratte, dagli strati precedenti, in modo 
...
architetture\index{architettura} che richiedono un input a lunghezza prefissata (es: CNN). Nelle 
...
connessione simile alle CNN\index{Convolutional Neural Netwrok - CNN} che li permette di retro propagare i dati in direzione 
opposta. Per capire meglio, immaginiamo che una RNN sia composta 
da un solo neurone\index{neurone} che, all'istante di tempo $t$, prende in input un dato $X_t$, 
...
neurone\index{neurone} quindi rappresenta una cella di memoria capace di preservare qualche stato nel 
...
\begin{enumerate}
    ...
    \item \emph{Delayed sequence to sequence}: in questa tipologia abbaino un'architettura\index{architettura} 
...
learning\index{deep learning}, ha portato vari benefici nel risolvere problemi complessi nella vita reale. 
...
\begin{itemize}
    ...
    di assegnare un'etichetta\index{etichetta} in base al suo contenuto;
    ...
    \item \emph{Emotion Recognition}: processo atto a identificare\index{identificare} le emozioni umane mediante 
...
maggiormente sull'object detection\index{object detection} e la semantic segmentation\index{semantic segmentation} (branca 
...
degli oggetti, rilevamento\index{rilevamento} degli eventi e riconoscimento delle attività. La tecnica 
di object detection supporta un ampio range di applicazioni, tra queste abbiamo 
la visione robotica, sicurezza, guida autonoma\index{autonoma}, segnali stradali, interazione uomo 
...
2012, periodo corrispondente all'introduzione del deep learning\index{deep learning}, le prestazioni 
della tecnica di object detection\index{object detection} aumentarono notevolmente.
\begin{figure}
    \centering
...
tutti i frame (tecnica comunemente chiamata object tracking). Il rilevamento\index{rilevamento} 
...
\begin{enumerate}
    ...
    un'etichetta\index{etichetta} corrispondente ad una classe (Fig. \ref{segmentation} (a));
...
aspetto fondamentale da tenere in considerazione è la complessità computazionale\index{computazionale} 
a cui i sistemi vanno incontro \cite{cyganek2013object}. I primi modelli\index{modello} di object detection basavano 
...
visti in precedenza. Le reti neurali convoluzionali (CNN)\index{Convolutional Neural Netwrok - CNN} e del deep learning\index{deep learning} hanno 
cambiato il panorama della percezione visiva. Il problema di object detection\index{object detection} è 
generalmente visto come un problema di apprendimento\index{apprendimento} supervisionato. Nella 
...
\begin{itemize}
    ...
    numero di etichette\index{etichetta} che a loro volta sono difficili da reperire. In questo caso 
    ...
    modello\index{modello} è una domanda di ricerca ancora aperta;
    \item \emph{Efficienza}: attualmente non è possibile eseguire un modello complesso su 
    dispostivi sprovvisti di elevate risorse\index{risorse} di calcolo. La ricerca è rivolta nell'ottimizzazione\index{ottimizzazione} 
...
Le prestazioni di un modello\index{modello} vengono calcolate utilizzando alcune metriche, come 
...
\begin{itemize}
    ...
    l'accettabilità o meno dell'etichetta\index{etichetta} della classe predetta. 
\end{itemize}
Una predizione $(b,c,p)$ è considerata come un vero positivo $(TP)$ se:
\begin{enumerate}
    \item La categoria predetta $c$ è uguale all'etichetta\index{etichetta} $c_g$ fornita nel ground truth;
    ...
    del modello\index{modello} in quanto non c'è sovrapposizione fra le varie bounding box. 
...
derivano dai progressi svolti nell'ottimizzazione\index{ottimizzazione} delle \emph{Deep Convolutional Neural 
Networks (DCNN)}, in particolare dalle Convolutional Neural Networks (CNN)\index{Convolutional Neural Netwrok - CNN}. 
...
la strada verso il continuo successo delle CNN\index{Convolutional Neural Netwrok - CNN} nello svolgere vari compiti di visione 
...
nella visione artificiale\index{visione artificiale} in quanto è utile in diversi campi scientifici, come per 
esempio la guida autonoma\index{autonoma}, ambito medico, ambito biometrico, ambito militare, 
riconoscimento etc. Per poter svolgere questo complicato compito, a differenza 
della classificazione e dell'object detection\index{object detection}, è necessario reperire delle informazioni 
...
verrà discussa solamente la tecnica di \index{semantic segmentation}. prima di introdurre 
...
Il deep learning\index{deep learning} ha fortemente promosso la ricerca sulla segmentazione semantica. 
...
stessa etichetta\index{etichetta}, condividono determinate caratteristiche come colore, intensità, 
...
tecniche, la tecnica di semantic segmentation\index{semantic segmentation} ha esigenze più elevate in quanto 
non si limita solamente a classificare gli oggetti presenti (image classification) e a 
rilevarne la loro posizione (object detection)\index{object detection} mediante delle bounding box, ma 
...
\end{figure}
Allo stato dell'arte esistono varie tipologie di architetture\index{architettura} di reti neurali 
...
convoluzionali, fino a giungere ai layer\index{layer} fully connected (FC). L'output prodotto 
sarà sotto forma di label predetta (Fig. \ref{CNN complete}). Per convertire questi modelli\index{modello} da 
classificatori a modelli FCN densi, bisogna sostituire i layer fully connected con 
layer\index{layer} convoluzionali di dimensione $1 \times 1 \times 21$, dove 21 rappresenta il numero delle 
...
\begin{enumerate}
    ...
    \item \emph{Up-sampling Bicubico}: Calcola il valore dei pixel attraverso l'interpolazione polinomiale. Questa operazione ha un alto tasso computazionale\index{computazionale} ma allo stesso tempo produce risultati migliori rispetto alle sue concorrenti. 
\end{enumerate}
Quando si sarà giunti a un layer\index{layer} di pooling pari alla dimensione di $1 \times 1$, allora 
...
\begin{figure}
    ...
    \label{FCN}
\end{figure}
Tale architettura\index{architettura} è composta da un \emph{Encoder} e da un \emph{Decoder}.
\begin{itemize}
    \item \emph{Encoder}: sezione dell'architettura\index{architettura} adibita all'estrazione del contesto dell'immagine. 
    La sua struttura è composta da livelli di convoluzione e di max 
    pooling, come una comune CNN\index{Convolutional Neural Netwrok - CNN}, che riducono (down-sample) l'immagine 
    in ingresso; 
    \item \emph{Decoder}: sezione dell'architettura\index{architettura} adibita alla localizzazione\index{localizzazione}  degli elementi 
    ...
    suoi artefatti.
\end{itemize}
Esitono diverse reti FCN che adottano questa tipologia di architettura\index{architettura} (es: \emph{Autoencoder}). 
...
\begin{figure}
    ...
    \label{deconvolution}
\end{figure}
In base al numero di layer\index{layer} convoluzionali presenti, esistono diverse architetture\index{architettura}
...
vengono in auto altre due tipologie di architetture\index{architettura}: \emph{FCN-16} e \emph{FCN-8}.
\begin{figure}
    \centering
    ...
    \label{FCN-models}
\end{figure}
Quest'ultime utilizzano una strategia intelligente\index{intelligenza} per poter ricostruire l'immagine. 
...
Per giungere ad un'architettura\index{architettura} FCN-8, bisognerà svolgere un processo 
iterativo che va ad effettuare una somma di ogni singolo elemento (element-wise) 
tra l'upsampling del pooling layer\index{layer} $n$ con il pooling layer $n-1$ (Fig. (\ref{FCN-models})). La ricostruzione 
risultante sarà quella più fedele al ground truth. Questa architettura\index{architettura} 
...
Per rimediare alla bassa qualità prodotta in output da una comune rete CNN\index{Convolutional Neural Netwrok - CNN}, Yu 
e Koltun introdussero un versione modificata chiamata {\bfseries{Convoluzione Dilatata}} 
o {\bfseries{DilatedNet}} \cite{DilatedNet}. Questo modello\index{modello} consentiva di estrarre le informazioni che 
...
DeepLab è un modello\index{modello} utilizzato allo stato dell'arte per la segmentazione semantica 
...
di layer\index{layer} di pooling che riducono la risoluzione spaziale delle feature map. 
...
campo visivo dei filtri senza aumentare il numero di parametri\index{parametro} preservando la 
...
relazione tra le etichette\index{etichetta} di pixel simili che esaltano il foreground dal background. 
...
L'architettura\index{architettura} generale di un modello\index{modello} DeepLab (Fig. (\ref{CRF})) consiste nel prendere in input 
...
map copiate dallo strato dell'encoder. Fra i vari layer\index{layer} esistenti in entrambe le 
...
encoder e un decoder. La parte adibita all'encoder utilizza 13 layer\index{layer} convoluzionali, 
mentre la parte destinata al decoder contiene anch'essa 13 layer per la deconvoluzione. 
In ogni layer\index{layer} della sezione encoder, viene utilizzato un banco di filtri 
...
sul punteggio di classificazione finale. Ogni feature map appartiene a un layer\index{layer} 
...
classificazione e localizzazione\index{localizzazione} . Quest'ultimi sono ritenuti come due concetti 
...
stesso tempo, la localizzazione\index{localizzazione}  potrebbe risentirne negativamente, pertanto risulta 
...
\begin{enumerate}
    ...
    completamente connessi (fully-connected layers)\index{layer}, o pooling globali, questi 
...
backbone (spina dorsale), pre-addestrata con le immagini contenute nel dataset\index{dataset} 
ImageNet. Sulle mappe dei punteggi di bassa risoluzione viene effettuata un'operazione 
di upsampling con un layer\index{layer} di deconvoluzione. Le mappe ottenute dalla 
...
La prima versione del modello\index{modello} DeepLab ha subito delle modifiche che hanno 
...
computazionale\index{computazionale}. Per ottenere ciò viene fattorizzata una convoluzione standard in 
...
$1/4$ dell'immagine di partenza. Per ridurre le dimensioni dei layer\index{layer} di pooling, 
...
la tecnica di Object detection\index{object detection}, i \emph{Frame al secondo (Frame Per Second - FPS)}. 
...
segmentation, localizzazione\index{localizzazione} , tracking e varie attività di riconoscimento. 
...
in maniera autonoma\index{autonoma}. Tra le tecniche più utilizzate per il rilevamento\index{rilevamento} 
...
il modello\index{modello} DEEP-DIG \cite{DEEP-DIG} che utilizza ResNet come backbone con una 
architettura\index{architettura} FCN con multipli passaggi di upscaling utili a poter interpolare 
un'immagine. 
\begin{figure}
    \centering
...
\begin{enumerate}
    ...
    \item \emph{Rilevamento\index{rilevamento} delle feature (Feature Detection)}: l'obiettivo principale 
...
rischio di incidenti. L'utilizzo delle sole telecamere\index{telecamere} comporta delle carenze 
...
sta spostando verso l'utilizzo di altri sensori quali telecamere\index{telecamere} con flash, 
...
collisioni con i veicoli, le telecamere\index{telecamere} installate sul veicolo servono per 
...
\begin{enumerate}
    ...
    altamente dinamiche in cui sia la telecamera\index{telecamere} e il pedone sono in 
...
e le reti neurali convoluzionali (CNN)\index{Convolutional Neural Netwrok - CNN}. Essendo questo uno studio incentrato 
sull'argomentazione delle reti CNN\index{Convolutional Neural Netwrok - CNN}, l'algoritmo di Viola Jones \cite{viola2001rapid} 
...
tramandati al layer\index{layer} finale Softmax, da cui ne susseguirà la classificazione. 
...
computer vision e grazie al supporto del deep learning\index{deep learning}, può essere in grado 
...
riguarda la localizzazione\index{localizzazione}  della segnaletica stradale nelle varie ROI, mentre la 
...
a basso impatto sull'accuratezza\index{accuratezza} di ogni modello\index{modello}. Ciò che si sta 
...
grazie agli evidenti miglioramenti delle prestazioni dei modelli. I benchmarks\index{benchmark} 
...
\begin{enumerate}
    \item \emph{Dynamic Quantization}: viene eseguita a runtime su un modello\index{modello} pre-addestrato. 
    ...
    essere la meno performante, ottenendo delle performance\index{performance} negative.
...
il kernel nel successivo layer\index{layer}, in un contesto structured ciò comporterebbe 
